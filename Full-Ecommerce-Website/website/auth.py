    ...
    if form.validate_on_submit():
        ...
        if customer:
            if customer.verify_password(password=password):
                ...
                # Check if customer.id == 1 and redirect accordingly
                if customer.id == 1:
                    return redirect('/dashboard')  # Redirect to dashboard if customer.id is 1
                else:
                    return redirect('/')  # Redirect to home page for other users
            else:
...
@login_required
def dashboard():
    # You can add additional logic here for the dashboard, e.g., fetching data
